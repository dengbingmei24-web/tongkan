...
## 14. Alpha 10.2.1 五项 P0/P1

- P0：添加视频改为底部面板，避免系统 AlertDialog 遮挡上下文。
- P0：客户端提交前逐条识别直链、B23、无效内容和数量上限。
- P0：服务端结果逐条展示 added/duplicate/rejected；B23 失败使用“失效或视频不可用”文案，不再只建议稍后重试。
- P1：片库默认按分类展示带封面的媒体分区；筛选、搜索和排序仍使用稳定单列表。
- P1：竖屏房间用底部片库抽屉，横屏/全屏用右侧抽屉；视频保持可见，匿名或未绑定状态继续保留手动链接流程。
- 实现仍为 Java View；新增受限 HTTPS 封面加载与内存缓存，不增加第三方依赖，不修改 Account API 或房间协议。
