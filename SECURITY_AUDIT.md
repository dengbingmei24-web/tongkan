...
- S-01 已于 2026-08-04 完成修复：移除 `<all_urls>`，构建时显式生成允许的同看 Web origins；桥接层校验 `event.origin`、房间标识、播放锚点和数值范围；B站及 B23 导航 URL 由受信字段重新规范化；新增恶意来源和伪造 URL 回归测试。
- S-02 已于 2026-08-04 完成修复：浏览器请求仅允许显式配置的 Web origins；创建房间按 Cloudflare 客户端 IP 使用独立 Durable Object 执行每分钟 20 次的固定窗口限流，并返回 `429` 与 `Retry-After`；新增跨域拒绝、预检与限流回归测试。
- S-03 / S-04 已于 2026-08-05 完成修复：房间从创建起或双方均离线后启动 10 分钟 Durable Object alarm，成员重连时取消，到期删除全部状态；直链拒绝凭据及常见签名、令牌类查询参数并移除 fragment，旧状态中的敏感直链在加载时自动清除；新增生命周期和 URL 隐私回归测试。
- S-05 已于 2026-08-05 完成修复：所有 WebSocket 客户端消息在进入认证和房间逻辑前执行 64 KiB 大小限制、严格字段白名单、必填字段、有限数值、媒体 URL 与嵌套 RTC schema 校验；昵称、聊天、SDP 和 ICE candidate 分别设置长度上限；新增异常 JSON、二进制、未知字段、极端数值和超长消息回归测试。
- S-06 已于 2026-08-05 完成修复：Durable Object 按成员席位与消息类别维护独立令牌桶，限制聊天、播放命令与报告、ping、屏幕共享和 RTC description / candidate 洪泛；正常操作保留突发额度，超限统一返回 `RATE_LIMITED`；新增突发、恢复、成员隔离、类别隔离和 Worker 门禁测试。
- S-07 仍为开放项，等待断线重连相关 Web 改动收口后处理。
